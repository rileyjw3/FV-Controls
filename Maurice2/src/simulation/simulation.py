...
class Simulation():
    ...
    def __init__(self):
        ...
        # CSV logging path
        root = Path(__file__).resolve().parents[2]  # …/Maurice2
        self.dynamics_path : str = root / "data" / "sim_output" / "dynamics"
        self.controls_path : str = root / "data" / "sim_output" / "controls"
    ...
    def run_controls_simulation(self, file_name : str = None, log_controls_moments: bool = True, show_progress: bool = True):
        ...
        output_path = self.controls_path / f"{file_name}.csv"
    ...
    def save_dynamics_to_csv(self, file_name: str):
        """Save the logged dynamics data to a CSV file.

        Args:
            file_name (str): File name to save the dynamics data as CSV.
        """
        ...
        dynamics_df.to_csv(file_name, index=False)


    def save_controls_to_csv(self, file_name: str):
        """Save the logged controls data to a CSV file.

        Args:
            file_name (str): File name to save the controls data as CSV.
        """
        ...
        controls_df.to_csv(file_name, index=False, mode='w')
